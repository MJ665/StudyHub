    ...
    <div className="chat-md text-sm leading-relaxed break-words">
      <ReactMarkdown
        ...
        components={{
          ...
          a: ({ children, href }) => (
            <a href={href} target="_blank" rel="noopener noreferrer" className="text-[var(--color-brand-primary)] underline hover:text-[var(--color-brand-primary)]">
              {children}
            </a>
          ),
          blockquote: ({ children }) => (
            <blockquote className="border-l-2 border-[var(--color-brand-primary)]/40 pl-3 my-3 text-[var(--color-on-surface-variant)] italic">{children}</blockquote>
          ),
          code: ({ inline, className, children }: any) =>
            inline ? (
              <code className="px-1.5 py-0.5 rounded bg-[var(--color-surface-container-high)] text-[var(--color-brand-primary)] text-[0.85em] font-mono">{children}</code>
            ) : (
          ...
          td: ({ children }) => <td className="px-3 py-2 border-b border-[var(--color-outline-variant)]">{children}</td>,
        }}
      >
        {content}
      </ReactMarkdown>
