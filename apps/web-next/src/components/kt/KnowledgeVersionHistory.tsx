    ...
    <div className="flex flex-col h-full bg-[var(--color-surface-container)]/50 backdrop-blur-xl border-l border-[var(--color-outline-variant)]">
      <div className="p-6 border-b border-[var(--color-outline-variant)] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[var(--color-warning)]/10 flex items-center justify-center text-[var(--color-warning)] border border-[var(--color-warning)]/20">
            <History size={20} />
          </div>
          <div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {isLoading ? (
          <div className="h-full flex items-center justify-center">
            <Loader2 className="animate-spin text-[var(--color-warning)]" size={32} />
          </div>
        ) : versions.length === 0 ? (
          ...
          versions.map((v, i) => (
            <motion.div
              ...
              className="group p-5 bg-[var(--color-surface-container)] border border-[var(--color-outline-variant)] rounded-2xl cursor-pointer hover:border-[var(--color-warning)]/50 transition-all relative overflow-hidden"
            >
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-1 bg-[var(--color-warning)]/10 text-[var(--color-warning)] text-[10px] font-black rounded border border-[var(--color-warning)]/20 uppercase">
                    v{v.version}
                  </span>
                ...
                <ChevronRight size={16} className="text-[var(--color-on-surface-variant)] group-hover:text-[var(--color-warning)] transition-colors" />
              </div>
