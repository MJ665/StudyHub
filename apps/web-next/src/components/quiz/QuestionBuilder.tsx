...
export default function QuestionBuilder({ questions, onChange }: { questions: any[]; onChange: (q: any[]) => void }) {
  ...
  const input = 'w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-white outline-none focus:ring-1 focus:ring-indigo-500/50';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-[520px]">
      ...
      <div className="space-y-4 max-h-[560px] overflow-y-auto pr-1 custom-scrollbar">
        ...
        {list.map((q, i) => (
          <div key={i} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3">
            <div className="flex items-center justify-between gap-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Q{i + 1}</span>
              <div className="flex items-center gap-2">
                <button onClick={() => setPreviewIdx(previewIdx === i ? null : i)} title="Preview" className="text-slate-500 hover:text-indigo-400"><Eye size={14} /></button>
                <button onClick={() => removeQ(i)} title="Remove" className="text-slate-500 hover:text-rose-400"><Trash2 size={14} /></button>
              </div>
            </div>
            ...
            {['mcq_single', 'mcq_multi', 'true_false'].includes(q.question_type) && (
              <div className="space-y-2">
                ...
                <button onClick={() => addOpt(i)} className="text-[11px] font-bold text-indigo-400 hover:text-indigo-300 flex items-center gap-1"><Plus size={12} /> Add option</button>
              </div>
            )}
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <input type="number" className="w-20 bg-slate-950 border border-slate-800 rounded-lg px-2 py-1.5 text-sm text-white" value={q.points ?? 1} min={1} onChange={(e) => patchQ(i, { points: Number(e.target.value) || 1 })} title="Points" />
              <label className="text-[11px] text-slate-400 flex items-center gap-1.5 cursor-pointer hover:text-indigo-400">
              ...
              <button onClick={() => { const u = prompt('Paste image URL'); if (u) patchQ(i, { media_urls: [...(q.media_urls || []), u] }); }}
                className="text-[11px] text-slate-400 hover:text-indigo-400">+ Paste image URL</button>
              {(q.media_urls || []).length > 0 && <span className="text-[10px] text-slate-500">{q.media_urls.length} image(s)</span>}
            </div>
        ...
        <button onClick={addQ} className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold flex items-center justify-center gap-2"><Plus size={16} /> Add question</button>
      </div>

      {/* Raw JSON pane */}
      <div className="flex flex-col">
        <div className="flex items-center gap-2 mb-2 text-slate-400 text-[11px] font-black uppercase tracking-widest"><Code2 size={13} /> Raw JSON</div>
        <div className="flex-1 rounded-xl overflow-hidden border border-slate-800 min-h-[440px]">
          <Editor
            height="440px"
