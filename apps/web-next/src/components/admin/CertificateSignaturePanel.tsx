...
export default function CertificateSignaturePanel() {
  ...
  if (loading) {
    return <div className="p-6 flex items-center gap-3 text-[var(--color-on-surface-variant)]"><Loader2 className="animate-spin" size={18} /> Loading certificate settings…</div>;
  }

  return (
    <div className="bg-[var(--color-surface-container)] border border-[var(--color-outline-variant)] rounded-2xl p-6 max-w-2xl">
      <div className="flex items-center gap-2 mb-1 text-[var(--color-brand-primary)]">
        <PenTool size={16} />
        <span className="text-[10px] font-black uppercase tracking-[0.2em]">Certificate Signatory</span>
      </div>
      <h3 className="text-lg font-bold text-[var(--color-on-surface)]">Organization signature</h3>
      <p className="text-xs text-[var(--color-on-surface-variant)] mt-1 mb-5">Appears on every certificate your learners earn (question banks &amp; exams).</p>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <button onClick={() => fileRef.current?.click()} disabled={uploading}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[var(--color-surface-container-high)] hover:bg-[var(--color-surface-bright)] border border-[var(--color-outline-variant)] text-sm text-[var(--color-on-surface)] disabled:opacity-50">
